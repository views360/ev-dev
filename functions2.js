...
function updateConclusionsAndItineraryUI(inputs, providers, publicKwh, totalAdhocCost, conclusionsBox) {
    ...
    const extraNote = `<p style="font-size:0.85rem; margin:0; opacity:0.8; color:var(--neon-green) !important;"><span style="opacity: 1">CAUTION:</span> Before purchasing a one-month subscription, check that your chosen provider has charging stations in your planned area(s) of travel — else your subscription will be wasted. Download the essential charging-location maps from the <a href="apps.html" style="color:var(--neon-green) !important;">EV App Toolkit</a> page. If the cheapest provider isn't available where you're going — or the provider's local rates are higher than their average rate if they offer location-specific rates, check the next cheapest provider, and so on.</p>`;
