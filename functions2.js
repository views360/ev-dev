...
function calculateRangeHtml(inputs, mainInitialRange) {
    ...
    if (inputs.additionalJourneys.length > 0) {
        ...
        rangeHtml += `<div style="font-size: 0.8rem; opacity: 0.5; margin-bottom: 2px; margin-left: 10px;">
            Journey 1 range: ${mainInitialRange.toFixed(0)} miles (from depature: ${j.soc}% down to recharge threshold: ${inputs.rechargeAt}%)
        </div>`;
        ...
        inputs.additionalJourneys.forEach((j, index) => {
            ...
            rangeHtml += `<div style="font-size: 0.8rem; opacity: 0.5; margin-bottom: 2px; margin-left: 10px;">
                Journey ${index + 2} range: ${extraRange.toFixed(0)} miles (from depature: ${j.soc}% down to recharge threshold: ${inputs.rechargeAt}%)
            </div>`;
        });
